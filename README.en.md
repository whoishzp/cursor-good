...
- **Message queue** — messages sent while AI is busy are marked "⏳ Pending", batched and sent together on the next AI turn; **the AI's current message is shown first**, then pending badges clear and the merged user input is returned
...
Download the latest `.vsix` from the [`vsix/`](https://github.com/whoishzp/cursor-good/tree/master/vsix) directory, e.g. `cursor-good-3.6.1.vsix`.
